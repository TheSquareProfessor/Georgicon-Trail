...
//show bugonia
OregonH.UI.showBugonia = function(products){
  ...
  //show products removed quant
  var product = ritual;
    sacsDiv.innerHTML += '<div class="product" data-qty="' + product.qty + '" data-item="' + product.item + '" data-price="' + product.price + '">' + product.item + '</div>';
...
//buy product slash click an answer
OregonH.UI.sacrifice = function(product) {
  //check we can afford it
  if(product.price > OregonH.UI.caravan.oxen) {
    OregonH.UI.notify('No oxen remaining', 'negative');
    return false;
  }

  OregonH.UI.caravan.oxen -= product.price;

  OregonH.UI.caravan.bees += +product.qty;

  OregonH.UI.notify('Generated ' + product.qty + ' bees ', 'positive');
